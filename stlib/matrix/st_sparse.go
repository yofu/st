...
func (co *COOMatrix) Query (row, col int) float64 {
    if row > co.size || col > co.size { return 0.0 }
...
func (co *COOMatrix) Add (row, col int, val float64) {
    if row > co.size || col > co.size { return }
    if rdata, rok := co.data[row]; rok {
...
func ReadMtx(fn string) (*CRSMatrix, error) {
    ...
    ind := 0
    for _, j := range lis {
        if strings.HasPrefix(j, "%") { ind++; continue }
        var words []string
...
func (cr *CRSMatrix) Query (row, col int) float64 {
    if row > cr.size || col > cr.size { return 0.0 }
    if cr.column[cr.row[row]] > col || cr.column[cr.row[row+1]-1] < col { return 0.0 }
    for c:=cr.row[row]; c<cr.row[row+1]; c++ {
        tmpcol := cr.column[c]
        if tmpcol > col { return 0.0 }
        if tmpcol == col { return cr.value[c] }
    }
    return 0.0
}
...
func (cr *CRSMatrix) SetFunc (row, col int, val float64, f func(float64)float64) float64 {
    ...
    update := func (index int) {
        ...
        cr.column = tmpcolumn
        cr.value  = tmpvalue
        for i:=row+1; i<cr.size+1; i++ { cr.row[i]++ }
        cr.nz++
    }
...
func (cr *CRSMatrix) FELower (vec []float64) []float64 {
    for row:=0; row<cr.size; row++ {
        if cr.column[cr.row[row]] > row { continue }
        for c:= cr.row[row]; c<cr.row[row+1]; c++ {
            tmpcol := cr.column[c]
            if tmpcol >= row { break }
...
func (ll *LLSMatrix) Query(row, col int) float64 {
    if row == col { return ll.diag[col].value }
