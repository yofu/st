...
func (frame *Frame) WeightDistribution(fn string) error {
	...
	if !check {
		...
		for _, n := range frame.Nodes {
			...
			if ind == 0 { // FOR BASEMENT
				continue
			}
			for i := 0; i < 2; i++ {
				val := frame.Wind.Qi[i][ind-1] / frame.Ai.Qi[i][ind]
				if val > 1.0 {
					n.Factor[i] *= val
				}
			}
			hx[ind] += n.Weight[2] * n.Factor[0]
			hy[ind] += n.Weight[2] * n.Factor[1]
		}
		qx := make([]float64, size-1)
		qy := make([]float64, size-1)
		for i := 0; i < size-1; i++ {
			for j := size - 1; j > i; j-- {
				qx[i] += hx[j]
				qy[i] += hy[j]
...
func (frame *Frame) WindPressure() (string, string, error) {
	...
	breadth := make([]float64, size)
	length := make([]float64, size)
	frame.Wind.Qi[0] = make([]float64, size)
	frame.Wind.Qi[1] = make([]float64, size)
	for i := 0; i < size; i++ {
		...
		first := true
		for _, j := range frame.Nodes {
			if j.Coord[2] > frame.Ai.Boundary[i+1] && j.Coord[2] < frame.Ai.Boundary[i+2] {
				if first {
					minx = j.Coord[0]
					maxx = j.Coord[0]
					miny = j.Coord[1]
		...
		breadth[i] = maxy - miny
		length[i] = maxx - minx
		wx[i] = q * cf[i] * (frame.Ai.Level[i+1] - frame.Ai.Level[i]) * breadth[i] * 0.001
		wy[i] = q * cf[i] * (frame.Ai.Level[i+1] - frame.Ai.Level[i]) * length[i] * 0.001
		for j := 0; j <= i; j++ {
			frame.Wind.Qi[0][j] += wx[i]
...
func (frame *Frame) SectionRateCalculation(fn string, long, x1, x2, y1, y2 string, sign float64, cond *Condition) error {
	...
	for _, el := range elems {
		...
		al, str, err := el.OutputRateInformation(long, x1, x2, y1, y2, sign, nil, 0.0)
		if err != nil {
			continue
		}
		otp.WriteString(str)
