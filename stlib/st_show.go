...
func (show *Show) Dataline() []string {
	...
	for i, ec := range []uint{EC_NUM, EC_SECT, EC_PREST, EC_STIFF_X, EC_STIFF_Y, EC_DRIFT_X, EC_DRIFT_Y, EC_TENSILE_X, EC_TENSILE_Y, EC_WIDTH, EC_HEIGHT, EC_SIZE} {
		if show.ElemCaption&ec != 0 {
			if ec == EC_PREST {
				...
				first = append(first, ELEMCAPTIONNAME[i]+fmt.Sprintf("[%s,%s^2]", show.UnitName[1], show.UnitName[1]))
			} else {
