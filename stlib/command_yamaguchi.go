...
func ring (stw *Window) {
    ...
    nodes := make(map[float64]*Node, num)
    sortednodes := make([]*Node, num)
    var keys []float64
    for _, n := range stw.SelectNode {
        if n != nil {
    ...
    for i:=0; i<num-1; i++ {
        stw.Frame.AddLineElem([]*Node{sortednodes[i], sortednodes[i+1]}, sec, COLUMN)
    }
    stw.Frame.AddLineElem([]*Node{sortednodes[num-1], sortednodes[0]}, sec, COLUMN)
    stw.EscapeAll()
}
// }}}
